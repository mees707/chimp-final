...
# and gives 'seq_G2R_output.csv' as output file
...
def user_input():
    print("On which state change does the ranking need to be based on: G2R, R2G or both")
    state_input = input("Please type G2R or R2G or both:    ")
...
print("Converting timestamps and grouping data...")
...
if state_input != 'both':
    for i in range(len(groups)):
        ...
        groups[i] = group[group['state'] == state_input]
        minmaxgroups[i] = mmgroup[mmgroup['state'] == state_input]
...
outputdf.to_csv('Output/output.csv')
